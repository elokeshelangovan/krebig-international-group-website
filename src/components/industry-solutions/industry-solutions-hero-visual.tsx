...
/**
 * Premium "hub and spoke" visual for the Industry Solutions hero — matches
 * the Divisions/About/How We Partner hero dashboard-panel visual language
 * (glass card, dot-grid, gradient orbs, floating chips) while representing
 * this page's own theme: many industries (satellite nodes) served by one
 * integrated approach (central hub).
 */
export function IndustrySolutionsHeroVisual() {
    ...
    >
      ...
      <div className="border-border bg-card/70 shadow-modal relative flex h-full w-full flex-col items-center justify-center overflow-hidden rounded-3xl border p-6 backdrop-blur-xl sm:p-8">
        <svg className="text-border absolute inset-0 h-full w-full opacity-40" aria-hidden="true">
          <defs>
            <pattern id="industry-hero-grid" width="24" height="24" patternUnits="userSpaceOnUse">
        ...
        >
          ...
          {SATELLITES.map((node, index) => (
            <m.line
              ...
              x2={node.cx}
              y2={node.cy}
              stroke="url(#industry-spoke-gradient)"
              strokeWidth="1.5"
              strokeLinecap="round"
        ...
        >
          ...
          <span className="text-xs font-semibold whitespace-nowrap">12 Industries</span>
        </m.div>
        ...
        >
          ...
          <span className="text-xs font-semibold whitespace-nowrap">Tailored Solutions</span>
        </m.div>
      </div>
    </div>
  );
