...
/**
 * Premium "hub and spoke" visual for the Divisions hero — no stock imagery.
 * Matches the homepage/About/How We Partner hero dashboard-panel visual
 * language (glass card, dot-grid, gradient orbs, floating chips) while
 * representing this page's own theme: specialized divisions (satellite
 * nodes) working together as one unified vision (central hub).
 */
export function DivisionsHeroVisual() {
    ...
    >
      ...
      <div className="border-border bg-card/70 shadow-modal relative flex h-full w-full flex-col items-center justify-center overflow-hidden rounded-3xl border p-6 backdrop-blur-xl sm:p-8">
        <svg className="text-border absolute inset-0 h-full w-full opacity-40" aria-hidden="true">
          <defs>
            <pattern id="divisions-hero-grid" width="24" height="24" patternUnits="userSpaceOnUse">
              <circle cx="1.5" cy="1.5" r="1.5" fill="currentColor" />
            </pattern>
          </defs>
          <rect width="100%" height="100%" fill="url(#divisions-hero-grid)" />
        </svg>
        ...
        >
          <defs>
            <linearGradient id="divisions-spoke-gradient" x1="0" y1="0" x2="1" y2="1">
          ...
          {SATELLITES.map((node, index) => (
            <m.line
              ...
              x2={node.cx}
              y2={node.cy}
              stroke="url(#divisions-spoke-gradient)"
              strokeWidth="1.5"
              strokeLinecap="round"
        ...
        >
          ...
          <span className="text-xs font-semibold whitespace-nowrap">4 Divisions</span>
        </m.div>
        ...
        >
          ...
          <span className="text-xs font-semibold whitespace-nowrap">One Unified Vision</span>
        </m.div>
      </div>
    </div>
  );
